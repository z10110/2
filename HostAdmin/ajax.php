...
switch ($_QET['act']) {
    case 'LoginVerification': //登录主机面板(唯一无需验证可放行接口)
    ...
    case 'SpaceDeleteSite': //删除站点
        ...
        if ($MainframeData['type'] == 1) {
            //删除网站+数据
            BTCO::GetDeleteSite($MainframeData['siteId'], $MainframeData['identification'] . '.com');
        }
        $re = $DB->delete('mainframe', ['id' => $UserData['id']]);
        if ($re) {
            Hook::execute('DeleteHostSpace', [
                'id' => $MainframeData['id'],
            ]);
    ...
    case 'SpaceDNSAdd': //新增域名
        ...
        $msg = [];
        if (!isset($Dass['status'])) {
            $SuccessList = []; //成功域名
            foreach ($Dass['domains'] as $key => $Domain) {
                if (!$Domain['status']) {
                    unset($_QET['domain'][$key]);
                }
                $SuccessList[] = $Domain['name'];
                $msg[] = '域名：' . $Domain['name'] . ' ' . $Domain['msg'];
            }
            if (count($_QET['domain']) >= 1) {
                foreach ($Data as $key => $val) {
                    if (!in_array($val['name'], $SuccessList)) {
                        unset($Data[$key]);
                    }
                }
                if (count($DomainArray) == 0) {
                ...
                userlog('域名绑定', '您绑定了' . count($SuccessList) . '个主机域名(' . implode(',', $SuccessList) . ')，主机ID：' . $UserData['id'] . '！', $UserData['uid']);
                $DB->update('mainframe', ['domain[JSON]' => $Array], ['id' => $UserData['id']]);
                dies(0, implode('<br>', $msg));
            } else {
                dier($Dass);
            }
        }
        ...
        if ($Dass['status'] === true) {
            userlog('域名绑定', '您绑定了' . count($_QET['domain']) . '个主机域名(' . implode(',', $_QET['domain']) . ')，主机ID：' . $UserData['id'] . '！', $UserData['uid']);
            $DB->update('mainframe', ['domain[JSON]' => $Array], ['id' => $UserData['id']]);
            dies(0, $Dass['msg']);
        } else {
    ...
    case 'SpaceSetBatchData': //批量操作文件
        if (empty($_QET['data']) || empty($_QET['type'])) dies(-1, '请填写完整！');
    ...
    case 'SpaceSetBatchPasteV': //粘贴冲突验证
        ...
        if (isset($Dirs['status']) && !$Dirs['status']) {
            BTCO::WriteException($UserData['id'], $Dirs['msg'], 2);
    ...
    case 'SpaceSetBatchPaste': //粘贴全部文件
        ...
        if ($Dirs['status'] === true) {
            dies(0, $Dirs['msg'] . (empty($_SESSION['BatchData']) ? ' | 复制' : ' | 剪切'));
        } else {
            BTCO::WriteException($UserData['id'], $Dirs['msg'], 2);
    ...
    case 'SpaceGetIndex': //默认文件列表获取
        ...
        if (!empty($Data)) {
            ...
            BTCO::WriteException($UserData['id'], '文件列表获取失败', 2);
            dies(-1, '文件列表获取失败');
        }
