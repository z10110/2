...
if (!empty($_QET['cid'])) {
    ...
    if (!$Class) {
        show_msg('分类->' . $_QET['cid'] . '不存在', '分类->' . $_QET['cid'] . '不存在请检查是否访问有误?');
    }
} else {
    $Class = [
        'state' => 1,
...
$ClassList = $DB->select('class', ['cid', 'name'], [
    'superior' => -1,
    'cid[!]' => $_QET['cid'] ?? 0,
]);
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="form-horizontal layui-form">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <div class="input-group">
                            <input type="text" name="image" lay-verify="required" lay-verType="tips"
                                   ...
                                   value="<?= $Cid === -1 ? href(2) . ROOT_DIR . 'assets/img/sc.jpg' : $Class['image'] ?>"
                    ...
                    <div class="form-group mb-3">
                        <label for="example-input-normal" style="font-weight: 500">配置上级分类</label>
                        <select class="custom-select mt-3" name="superior">
                            <option value="-1" <?= $Class['superior'] == -1 ? 'selected' : null ?>>设置为一级分类</option>
                            <?php foreach ($ClassList as $value) { ?>
                                <option value="<?= $value['cid'] ?>" <?= $Class['superior'] == $value['cid'] ? 'selected' : null ?>
                                >[CID:<?= $value['cid'] ?>]<?= $value['name'] ?></option>
                            <?php } ?>
                        </select>
                        <div style="font-size:14px;color: #444444;margin-top: 5px;">
                            注意事项：主分类降级会同步调整子下级分类的上级分类ID，列如：分类1是一级分类，分类1下面有3个二级分类绑定了分类1，当分类1降级为分类8的二级分类的话，分类1下面的3个二级分类全部一起跟随变为分类8的下级分类，如果是升级为一级分类的话，则会自动脱离上级分类，如果上级分类被删除，则被删除的分类下级全部升级为一级分类！
                        </div>
                    </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="number" name="grade" lay-verify="required" class="form-control"
                               value="<?= $Class['grade'] ?? 1 ?>" placeholder="多少级的用户才可以看到此分类？">
                        <div style="font-size:14px;color: #444444;margin-top: 5px;">
