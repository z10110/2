    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                ...
                <a href="javascript:App.ResetSort()" class="btn btn-danger badge ml-1 pb-0"
                   style="padding-bottom: 0.3em !important;">重置排序ID</a>
            </div>
            <div class="card-body" id="App">
                <div class="table-responsive" id="table">
                    <table class="table table-hover table-centered mb-0" style="font-size:0.9em;">
                        ...
                        <tbody>
                        <tr v-for="(item,index) in Data" :class="'Tab_'+item.superior"
                            ...
                            <td>
                                <input :onchange="'App.SortSet('+item.cid+',this)'" type="number" style="width: 50px;"
                                       class="layui-input"
                                       :value="item.sort">
                            </td>
                            <td>
                            ...
                            <td>
                                <span v-if="item.superior==-1" style="color: #999"><i
                                            :id="'Tab_'+item.cid"
                                            class="layui-icon layui-icon-right"
                                            @click="Deployment(item.cid,item.SuborderNumber)"
                                            v-if="item.SuborderNumber>=1"
                                            style="color:rgb(61,30,200);font-size: 90%;cursor: pointer">展开({{ item.SuborderNumber }})</i>
                                            <span v-else>—</span>
                                </span>
                                <span v-else class="badge badge-primary-lighten">{{ item.superior }}</span>
                            </td>
                            <td>
                                <span class="badge badge-primary-lighten" @click="BatchSetting(item.cid)"
                                      style="cursor: pointer;">{{item.count}}个(设置)</span>
                            </td>
                            <td>
...
<script src="../assets/admin/js/classlist.js?vse=<?= $accredit['versions'] ?>"></script>
</body>
</html>
