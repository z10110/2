            ...
            <div class="card-body mdui-p-a-2">
                ...
                <div id="UnderlyingConfiguration" class="mdui-p-a-2">
                    ...
                    <a href="http://cloud.79tian.com/" target="_blank"
                       class="mdui-btn mdui-btn-icon mdui-text-color-light-blue"
                       mdui-tooltip="{content: '云储-官方免费网盘'}">
                        <i class="mdui-icon material-icons">&#xe2be;</i>
                    </a>
                    <div class="mdui-textfield">
                ...
                <div id="OrderInformation" class="mdui-p-a-2">
                    <div class="mdui-list">
                        ...
                        <label v-show="Form.specification===true"
                            ...
                            <div class="mdui-list-item-content" style="font-size:0.9em;">
                                将用户的规格选择参数提交至对接站
                            </div>
                            <label class="mdui-switch">
                    ...
                    <div class="mdui-textfield" id="input">
                        ...
                        <div class="mdui-textfield-helper">
                            可留空,可点击下方按钮生成输入框,或预览生成效果，留空显示为：下单账号
                        </div>
                    </div>
                ...
                <div id="PriceConfiguration" class="mdui-p-a-2">
                    ...
                    <!--绑定商品规格数据-->
                    <div class="mdui-textfield" v-if="ListRule">
                        <label style="font-width: 200 !important;font-size: 14px;">绑定加价规则 <a
                                    href="./admin.PriceRule.php"
                                    target="_blank">[配置]</a> <a
                                    href="javascript:App.RuleDataGet(2)">[更新]</a></label>
                        <select v-model="Form.profit_rule" class="mdui-select" style="width: 100%;">
                            <option :value="-1" :selected="Form.profit_rule==-1?true:false">关闭加价规则</option>
                            <option v-for="(item,index) in ListRule" :selected="Form.profit_rule==item.id?true:false"
                                    :value="item.id">[ {{ item.id }} ] {{ item.name }}
                            </option>
                        </select>
                    </div>

                    <div class="mdui-textfield" v-show="Form.profit_rule==-1">
                        ...
                        <div v-if="Form.profits==100" style="color: red;font-size: 12px;">
                            当利润比例为100,并且没有绑定单商品加价规则时,商品售价会根据「<a
                                    href="admin.increasePrice.list.php" target="_blank">加价规则列表</a>」内的规则进行价格计算!，从而导致当前页面的各等级价格预览结果和实际售价不一致！
                        </div>
                    </div>
                ...
                <div id="ClassificationGoods" class="mdui-p-a-2">
                    ...
                    <div class="mdui-textfield">
                        <label class="mdui-textfield-label">默认商品参数配置[部分模板可用]</label>
                        <textarea v-model="Form.extend.parameters_default" class="mdui-textfield-input"></textarea>
                        <div class="mdui-textfield-helper">
                            如果留空，则显示全部默认商品参数，可填写内容：商品编号,商品销量,剩余库存,每份数量,最少购买,最多购买,当前等级,商品售价,消耗
                        </div>
                    </div>
                    <div class="mdui-textfield" id="parameters">
                        <label class="mdui-textfield-label">自定义商品参数</label>
                        <textarea v-model="Form.extend.parameters" class="mdui-textfield-input"></textarea>
                        <div class="mdui-textfield-helper">
                            可留空或点击下方按钮生成自定义参数,或预览生成效果，仅对指定模板生效
                        </div>
                    </div>
                    <button @click="ParametersAdd()" class="mdui-btn mdui-btn-icon mdui-text-color-orange"
                            mdui-tooltip="{content: '添加一个新的商品参数'}">
                        <i class="mdui-icon material-icons">&#xe148;</i>
                    </button>
                    <button @click="ParametersPreview()" class="mdui-btn mdui-btn-icon mdui-text-color-teal"
                            mdui-tooltip="{content: '参数预览'}">
                        <i class="mdui-icon material-icons">&#xe3b4;</i>
                    </button>
                    ...
                    <div class="card mdui-m-b-0 mdui-m-t-2">
                        <div class="card-header mdui-p-a-1">
                            <a title="初始化" href="javascript:App.ClassDataGet(2);"
                               class="badge badge-warning-lighten"><i
                                        class="layui-icon layui-icon-refresh-3"></i></a>
                            选择商品分类
                        </div>
                            ...
                            <div v-for="(item,index) in DockingList" @click="ChooseDocking(index)"
                                ...
                                <div class="card mdui-ripple">
                                    <div class="card-header layui-elip font-13">
                                        <button title="点击获取可对接商品列表"
                                                class="btn btn-outline-primary badge mr-1">
                                            {{ item.id }}
                                        </button>
                                        {{item.name}}
                                    </div>
...
price: 订单付款金额</pre>
                            ...
                            <div class="mdui-panel-item">
                                ...
                                <div class="mdui-panel-item-body">
                                    ...
                                    <b>kami</b>：可返回相关的卡密信息，数组格式，一行一卡，如：<span
                                            class="mdui-text-color-pink">["卡1","卡2","卡3"]</span><br>
    ...
    "message": "商品购买失败，余额不足！",
                        ...
                        <div class="mdui-panel" mdui-panel>
                            <div class="mdui-panel-item mdui-panel-item-open">
                                ...
                                <div class="mdui-panel-item-body">
                                    ...
                                    <p>
                                        4、配置完成后，点击上方的两个输入框，复制内容，然后最小化窗口，将复制的内容，填写到对应的输入框内！</p>
                                    ...
                                    <p>
                                        可通过配置【每份商品的发货数量】来调整，1=1月(30天)，用户可通过购买多份，来选择开通时长，以及价格等！</p>
                ...
                <div id="sundry" class="mdui-p-a-2">
                    <div class="mdui-list">
                        ...
                        <label mdui-tooltip="{content: '是否允许用户通过积分兑换此商品？'}"
                               class="mdui-list-item mdui-ripple">
...
<script src="../assets/admin/js/goodsadd.js?vse=<?= $accredit['versions'] ?>"></script>
