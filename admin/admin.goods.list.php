...
global $DB;
?>
<div class="row" id="AppHis" cid="<?= ($_QET['cid'] ?? -2) ?>" sqid="<?= ($_QET['sqid'] ?? -1) ?>">
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <select class="custom-select" name="GoodsState" style="width:10em;">
                    <option value="-1" selected="">全部商品 (<?= $DB->count('goods') - 0; ?>个)</option>
                    <option value="1">上架中 (<?= $DB->count('goods', ['state' => 1]) - 0; ?>个)</option>
                    <option value="2">已下架 (<?= $DB->count('goods', ['state' => 2]) - 0; ?>个)</option>
                    <option value="3">未分类 (<?= $DB->count('goods', ['cid' => -1]) - 0; ?>个)</option>
                    <option value="4">无库存 (<?= $DB->count('goods', ['quota' => 0]) - 0; ?>个)</option>
                    <option value="5">串货商品 (<?= $DB->count('goods', ['deliver' => -1]) - 0; ?>个)</option>
                    <option value="6">自营商品 (<?= $DB->count('goods', ['deliver' => 1]) - 0; ?>个)</option>
                    <option value="7">发卡商品 (<?= $DB->count('goods', ['deliver' => 3]) - 0; ?>个)</option>
                    <option value="8">Api对接 (<?= $DB->count('goods', ['deliver' => 2]) - 0; ?>个)</option>
                    <option value="9">宝塔主机 (<?= $DB->count('goods', ['deliver' => 5]) - 0; ?>个)</option>
                    <option value="10">隐藏内容发货 (<?= $DB->count('goods', ['deliver' => 4]) - 0; ?>个)</option>
                </select>
                ...
                <ul class="mdui-menu" id="BatchOperation">
                    ...
                    <li class="mdui-menu-item">
                        <a href="javascript:App.BatchEditor(12);">
                            加价规则
                        </a>
                    </li>
                    <li class="mdui-menu-item">
                        <a href="javascript:App.BatchEditor(13);">
                            批量换图
                        </a>
                    </li>
                    <li class="mdui-menu-item">
                        <a style="font-size:0.8em;" href="javascript:App.BatchEditor(14);">
                            图片本地化
                        </a>
                    </li>
                </ul>
                ...
                <a href="admin.goods.sort.php" class="btn btn-danger badge ml-1 pb-0" style="">排序</a>
            </div>
            <div class="card-body m-t-0" id="App" style="overflow-y: auto;min-height:70vh;">
                <div v-if="cid!=-2" class="mb-2">正在查看分类[ {{cid==-1?'未分类':cid}} ]下的全部商品 <a
                            href="admin.goods.list.php">查看全部</a></div>

                <div v-if="sqid!=-1" class="mb-2">正在查看货源[ id:{{sqid}} ]下的全部商品 <a
                            href="admin.goods.list.php">查看全部</a></div>

                <div v-if="state!=-1" class="mb-2">正在查看特定类型的商品列表 <a
                            href="admin.goods.list.php">查看全部</a></div>
                ...
                <table id="table" class="table table-hover table-centered mb-0" style="font-size:0.9em;">
                    ...
                    <tr>
                        ...
                        <th>排序值 <a href="javascript:layer.alert('排序值越高，越靠前！',{icon:3})">[注]</a></th>
                        <th>名称</th>
                        <th>所属分类</th>
                        <th>加价规则</th>
                        <th>商品图</th>
                    ...
                    <tr v-for="(item,index) in Data">
                        <td>
                            ...
                            <ul class="mdui-menu" :id="'html_operation_'+item.gid">
                                ...
                                <li class="mdui-menu-item">
                                    <a href="javascript:" :onclick="'goods.PriceSynchro('+item.gid+')'"
                                       class="mdui-ripple">价格同步</a>
                                </li>
                                <li class="mdui-menu-item">
                                    <a :href="'admin.order.list.php?gid='+item.gid" class="mdui-ripple">商品订单</a>
                                </li>
                                <li class="mdui-menu-item">
                                    <a href="javascript:"  @click="ReplaceProductImages([item.gid],item.image)"
                                       class="mdui-ripple">替换图片</a>
                                </li>
                                <li class="mdui-menu-item">
                                    <a href="javascript:" @click="BatchProductPictureTransfer([item.gid])"
                                       class="mdui-ripple" style="font-size:0.8em;">图片本地化</a>
                                </li>
                                <li class="mdui-menu-item">
                        ...
                        <td>
                            <input :onchange="'App.SortSet('+item.gid+',this)'" type="number" style="width: 50px;"
                                   class="layui-input"
                                   :value="item.sort">
                        </td>
                        <td>
                        ...
                        <td>
                            <div class="layui-elip" :title="item.Cname">
                                <a :href="'admin.goods.list.php?cid='+item.cid" title="点击查看此分类下的全部商品!"
                                >( {{item.cid}} )</a> <span v-html="item.Cname"></span>
                            </div>
                        </td>
                        <td>
                            <a href="admin.PriceRule.php" target="_blank" v-if="item.profit_rule==-1">未配置</a>
                            <span v-else><span @click="RuleNameTips(item.profit_rule,item.cost,item.name)"
                                               style="cursor: pointer;color: #0a6ebd"
                                >{{ RuleName(item.profit_rule) }}</span></span>
                        </td>
                        <td>
                            <img :src="item.image[0]" style="width:30px;height:30px;"/>
                        </td>
                        <td><span class="badge badge-primary-lighten" :onclick="'goods.PriceSynchro('+item.gid+')'"
                                  style="cursor: pointer" title="同步商品状态">{{item.money }}</span></td>
                        ...
                        <td>
                            ...
                            <a :href="'admin.goods.token.php?gid='+item.gid" title="点击补卡"
                               v-else-if="item.deliver==3"
                            ...
                            <a style="cursor: pointer;" :href="'?sqid='+item.shequID" v-else
                               class="badge badge-primary-lighten"
                               :title="item.sqid">对接：{{ item.sqid_name }}</a>
                        </td>
                        <td>
                            ...
                            <a v-if="item.state==2" href="javascript:void(0)"
                               @click="GoodsStateSet(item.gid,1,item.name)"
                               class="badge badge-danger-lighten">已下架</a>
                            <a v-if="item.state==3" href="javascript:void(0)"
                               @click="GoodsStateSet(item.gid,1,item.name)"
                               class="badge badge-warning-lighten">已隐藏</a>
                        </td>
                        <td>
...
<script src="../assets/admin/js/goodslist.js?vst=<?= $accredit['versions'] ?>"></script>
