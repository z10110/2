    ...
    <div class="card" style="width:100%">
        <div class="card-header">
            <select v-model="state" class="custom-select" name="OrderState" style="width:10em;">
                <option value="" selected="">查看全部(<?= $DB->count('order') - 0 ?>)</option>
                <option value="1">已完成(<?= $DB->count('order', ['state' => 1]) - 0 ?>)</option>
                <option value="2">待处理(<?= $DB->count('order', ['state' => 2]) - 0 ?>)</option>
                <option value="3">异常(<?= $DB->count('order', ['state' => 3]) - 0 ?>)</option>
                <option value="4">处理中(<?= $DB->count('order', ['state' => 4]) - 0 ?>)</option>
                <option value="5">已退款(<?= $DB->count('order', ['state' => 5]) - 0 ?>)</option>
                <option value="6">投诉中(<?= $DB->count('order', ['state' => 6]) - 0 ?>)</option>
                <option value="7">已评价(<?= $DB->count('order', ['state' => 7]) - 0 ?>)</option>
                <option value="">------</option>
                <option value="8">对接发货成功(<?= $DB->count('order', ['docking' => 1]) - 0 ?>)</option>
                <option value="9">对接发货失败(<?= $DB->count('order', ['docking' => 2]) - 0 ?>)</option>
                <option value="10">待提交对接发货(<?= $DB->count('order', ['docking' => 3]) - 0 ?>)</option>
                <option value="">------</option>
                <option value="11">已确认收货(<?= $DB->count('order', ['take' => 2]) - 0 ?>)</option>
                <option value="12">未确认收货(<?= $DB->count('order', ['take' => 1]) - 0 ?>)</option>
                <option value="">------</option>
                <option value="13">主站订单(<?= $DB->count('order', ['muid' => -1]) - 0 ?>)</option>
                <option value="14">分站订单(<?= $DB->count('order', ['muid[>=]' => 1]) - 0 ?>)</option>
            </select>
            ...
            <div class="mdui-textfield mdui-textfield-expandable mdui-float-right">
                ...
                <input class="mdui-textfield-input" v-model="name" type="text"
                       placeholder="可输入下单信息,订单号,订单ID"/>
        ...
        <div class="card-body m-t-0" id="App" style="overflow-y: auto">
            <div v-if="gid!=''" class="mb-2">正在查看商品编号为[ {{gid}} ]的全部订单 <a href="./admin.order.list.php">查看全部</a>
            </div>

            <div v-if="uid!=''" class="mb-2">正在查看用户[ {{uid}} ]的相关订单 <a
                        href="./admin.order.list.php">查看全部</a></div>
                ...
                <tr v-for="(item,index) in Data">
                    <td>
                        ...
                        <ul class="mdui-menu mdui-menu-cascade" style="font-size: 1em;" :id="'html_operation_'+item.id">
                            ...
                            <li class="mdui-menu-item">
                                <a href="javascript:" @click="OrderSet(1,item)" class="mdui-ripple">标记已完成</a>
                            </li>
                            <li class="mdui-menu-item">
                                <a href="javascript:" @click="OrderSet(2,item)" class="mdui-ripple">标记待处理</a>
                            </li>
                            <li class="mdui-menu-item">
                                <a href="javascript:" @click="OrderSet(3,item)" class="mdui-ripple">标记异常中</a>
                            </li>
                            <li class="mdui-menu-item">
                                <a href="javascript:" @click="OrderSet(4,item)" class="mdui-ripple">标记处理中</a>
                            </li>
                            <li class="mdui-menu-item">
                                <a href="javascript:" @click="OrderSet(5,item)"
                                   class="mdui-ripple">标记已退款</a>
                            </li>
                            <li class="mdui-menu-item">
                                <a href="javascript:" @click="OrderSet(6,item)" class="mdui-ripple">标记售后中</a>
                            </li>
                            <li class="mdui-menu-item">
                                <a href="javascript:" @click="OrderSet(7,item)" class="mdui-ripple">标记已评价</a>
                            </li>
                            <li class="mdui-divider"></li>
                            <li class="mdui-menu-item">
                            ...
                            <li class="mdui-menu-item">
                                <a href="javascript:" @click="OrderSet(8,item)" class="mdui-ripple">删除订单</a>
                            </li>
                        </ul>
                    </td>
                    <td>
                        ...
                        {{item.id}}
                        <span @click="OrderSet(5,item,2)" style="color: red;cursor: pointer"
                              v-if="item.state!=5&&item.return=='此对接订单已退单,请手动退单!'">[退单]</span>
                        </a>
                    </td>
                    <td>
                    ...
                    <td>
                        <div v-if="item.input&&item.InputName" @click="set_order(item)"
                             v-for="(vs,is) in item.input"
                             style="font-size:14px;color:#4c415f">
                            {{
                            (item.InputName[is]==''||item.InputName[is]==null?'输入框'+(is+1):item.InputName[is])
                            }} : {{vs}}
                        </div>
                        <div v-else style="font-size:12px;color:#ff9c0c">此订单无下单信息</div>
                    </td>
                    ...
                    <td>
                            <span class="badge badge-warning-lighten"
                                  v-if="item.payment=='积分兑换'||item.payment=='免费领取'">
