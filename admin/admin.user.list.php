        ...
        <div class="card">
            <div class="card-header">
                <select v-model="GradeIndex" class="custom-select" name="OrderState" style="width:10em;">
                    <option value="-1" selected="">查看全部(<?= $DB->count('user') - 0 ?>)</option>
                    <option v-for="(item,index) in GradeList" :value="(index>=0?index+1:index)">{{ item.name
                        }}({{item.count}})
                    </option>
                </select>
                <select v-model="UserSort" class="custom-select" name="OrderState" style="width:10em;">
                    ...
                    <option value="5" title="近，指的是近期，距离当前时间很近！">权限到期(远到近)</option>
                    <option value="6" title="近，指的是近期，距离当前时间很近！">权限到期(近到远)</option>
                    <option value="7" title="近，指的是近期，距离当前时间很近！">最近登录(远到近)</option>
                    <option value="8" title="近，指的是近期，距离当前时间很近！">最近登录(近到远)</option>
                </select>
                    ...
                    <tr v-for="(item,index) in Data">
                        <td>
                            ...
                            <ul class="mdui-menu" :id="'html_operation_'+item.id">
                                ...
                                <li class="mdui-menu-item">
                                    <a @click="EndTimeSet(item.id,item.endtime)"
                                       class="mdui-ripple">设置权限到期时间</a>
                                </li>
                                <li class="mdui-menu-item">
                        ...
                        <td @click="EndTimeSet(item.id,item.endtime)">
                            <span style="color: green" v-if="!item.endtime">永久</span>
...
<script src="../assets/admin/js/userlist.js?vst=<?= $accredit['versions'] ?>"></script>