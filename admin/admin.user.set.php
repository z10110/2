...
$BindingDomain = explode(',', $conf['BindingDomain']);
?>
<style>
    label {
        font-weight: 400;
        ...
        <div class="card">
            ...
            <div class="layui-form card-body mdui-p-a-0">
                <div id="Se1" class="mdui-p-a-2">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select class="custom-select mt-3" lay-search name="usergradecost">
                            <option <?= $conf['usergradecost'] == 1 ? 'selected' : '' ?> value="1">升级提成奖励给上级
                            </option>
                    ...
                    <div class="form-group mb-3">
                        <label>是否启用权限到期时间</label>
                        <select class="custom-select mt-3" lay-search name="PermissionTime">
                            <option <?= $conf['PermissionTime'] == 1 ? 'selected' : '' ?>
                                    value="1">启用,下方两个配置参数生效
                            </option>
                            <option <?= $conf['PermissionTime'] == 2 ? 'selected' : '' ?>
                                    value="2">不启用,升级到分站等级后,权限永久可用
                            </option>
                        </select>
                    </div>

                    <div class="form-group mb-3">
                        <label>权限续期价格百分比</label>
                        <input type="number" name="PermissionRenewPrice" class="form-control"
                               value="<?= $conf['PermissionRenewPrice'] ?>" placeholder="请填写完整">
                        <div class="mt-1" style="color: red">如设置80，则到期后只需要花费80%的钱，就可以续期对应等级！如5级需要100,续期就只需要花费80元！
                        </div>
                    </div>

                    <div class="form-group mb-3">
                        <label>购买权限后的权限有效期</label>
                        <input type="number" name="PermissionLengthTime" class="form-control"
                               value="<?= $conf['PermissionLengthTime'] ?>" placeholder="请填写完整">
                        <div class="mt-1" style="color: red">如填写365,则用户购买分站权限后,权限有效期为365天,到期后每次续费,也是续费365天
                        </div>
                    </div>
                ...
                <div id="Se2" class="mdui-p-a-2">
                    ...
                    <div class="form-group mb-3">
                        <label>分站域名验证模式 <span style="color: red">[仅泛解析模式生效]</span></label>
                        <select class="custom-select mt-3" lay-search name="StoreVerificationMode">
                            <option <?= $conf['StoreVerificationMode'] == 1 ? 'selected' : '' ?>
                                    value="1">宽松模式,仅通过域名前缀来验证,不限制域名主体
                            </option>
                            <option <?= $conf['StoreVerificationMode'] == 2 ? 'selected' : '' ?>
                                    value="2">严格验证,必须域名前缀和主体保持一致才判定为有效分站域名
                            </option>
                        </select>
                    </div>
                    <div class="form-group mb-3">
                        <label>自定义用户后台对接API</label>
                        <input type="text" name="CustomAPI" class="form-control" value="<?= $conf['CustomAPI'] ?>"
                               placeholder="可留空,维持现状！"/>
                        <span style="color:red">用户后台对接API地址默认显示为当前用户访问地址,留空显示默认,如果配置了参数，则显示参数内容！</span>
                    </div>
                    <div class="form-group mb-3">
                    ...
                    <div class="form-group mb-3">
                        <label>主站额外可访问域名</label>
                        <input type="text" name="BindingDomain" class="form-control"
                               value="<?= $conf['BindingDomain'] ?>"
                               placeholder="请填写完整"/>
                        <span style="color: rgba(253,160,39,0.87)">当主站需要额外绑定域名时,可以配置此处，每个域名使用【,】分割，仅在禁止了【是否允许用户访问其他未绑定的分店域名】配置项时有效</span>
                    </div>
                    <div class="form-group mb-3">
                    ...
                    <div class="form-group mb-3">
                        ...
                        当前默认放行域名：<a target="_blank"
                                    ...
                                    target="_blank"
                                    href="<?= is_https(false) . 'www.' . $accredit['url'] ?>">www.<?= $accredit['url'] ?></a>
                            <?php foreach ($BindingDomain as $value) { ?>
                                ，<a target="_blank"
                                    href="<?= is_https(false) . 'www.' . $value ?>">www.<?= $value ?></a>，
                                    <a target="_blank" href="<?= is_https(false) . $value ?>"><?= $value ?></a>，
                            <?php } ?>
                            <br>
                ...
                <div id="Se3" class="mdui-p-a-2">
                    ...
                    <div class="form-group mb-3">
                        <label for="example-input-normal">卡密充值开关</label>
                        <select class="custom-select mt-3" lay-search name="karmaRechargeSwitch">
                            <option <?= $conf['karmaRechargeSwitch'] == 1 ? 'selected' : '' ?> value="1">开启卡密充值功能
                            </option>
                            <option <?= $conf['karmaRechargeSwitch'] == -1 ? 'selected' : '' ?> value="-1">关闭卡密充值功能
                            </option>
                        </select>
                    </div>
                ...
                <div id="Se4" class="mdui-p-a-2">
                    ...
                    <div class="form-group mb-3">
                        <label for="example-input-normal">用户账号找回开关</label>
                        <select class="custom-select mt-3" name="AccountRetrieval">
                            <option <?= $conf['AccountRetrieval'] == 1 ? 'selected' : '' ?> value="1">开启用户账号找回
                            </option>
                            <option <?= $conf['AccountRetrieval'] == -1 ? 'selected' : '' ?> value="-1">关闭用户账号找回
                            </option>
                        </select>
                        <div class="mt-1" style="color: red">账号找回地址：<a
                                    href="javascript:FindPassword('<?= href(2) . ROOT_DIR_S . '/?mod=Find' ?>')"
                            ><?= href(2) . ROOT_DIR_S . '/?mod=Find' ?></a>
                        </div>
                    </div>
                    <div class="form-group mb-3">
                        <label for="example-input-normal">用户账户密码登录/注册</label>
                        <select class="custom-select mt-3" name="AccountPasswordLogin">
                            <option <?= $conf['AccountPasswordLogin'] == 1 ? 'selected' : '' ?> value="1">开启账号密码登录/注册
                            </option>
                            <option <?= $conf['AccountPasswordLogin'] == 2 ? 'selected' : '' ?> value="2">关闭账号密码登录/注册
                            </option>
                        </select>
                    </div>
                    <div class="form-group mb-3">
                        <label for="example-input-normal">用户后台手机号登录/注册</label>
                        <select class="custom-select mt-3" lay-search name="sms_switch_user">
                            <option <?= $conf['sms_switch_user'] == -1 ? 'selected' : '' ?> value="-1">关闭手机号登录/注册
                            </option>
                            <option <?= $conf['sms_switch_user'] == 1 ? 'selected' : '' ?> value="1">开启手机号登录/注册
                            </option>
                        </select>
                    </div>
                    <div class="form-group mb-3">
                    ...
                    <div class="form-group mb-3">
                        <label>账号注册时是否强制绑定手机号</label>
                        <select class="custom-select mt-3" lay-search name="RegisterPhone">
                            <option <?= $conf['RegisterPhone'] == 1 ? 'selected' : '' ?> value="1">开启强制绑定手机号
                            </option>
                            <option <?= $conf['RegisterPhone'] == -1 ? 'selected' : '' ?> value="-1">关闭强制绑定手机号
                            </option>
                        </select>
                        <div class="mt-1" style="color: red">必须开启手机号登录和账号密码登录才可以使用强制绑定功能！
                        </div>
                    </div>
                    <div class="form-group mb-3">
                        <label for="example-input-normal">强制用户登录</label>
                        <select class="custom-select mt-3" name="ForcedLanding">
                            <option <?= $conf['ForcedLanding'] == 1 ? 'selected' : '' ?> value="1">不开启用户强制登录
                            </option>
                            <option <?= $conf['ForcedLanding'] == 2 ? 'selected' : '' ?>
                                    value="2">开启用户强制登录(打开首页就跳转到登录界面)
                            </option>
                            <option <?= $conf['ForcedLanding'] == 3 ? 'selected' : '' ?>
                                    value="3">开启用户强制登录(只在下单的时候才提示需要登录)
                            </option>
                        </select>
                        <div class="mt-1" style="color: red">开启后用户必须登录才可以购买商品！
                        </div>
                    </div>
                    <div class="form-group mb-3">
                    ...
                    <div class="form-group mb-3">
                        <label>用户每日登录用户后台短信发送次数限制</label>
            ...
            <div class="card-body">
                ...
                <fieldset class="layui-elem-field">
                    ...
                    <div class="layui-field-box">
                        ...
                        <hr>
                        3、如果觉得以上解释较为复杂，可以直接点击<a target="_blank"
                                                                  href="./admin.level.list.php">等级列表</a>内的<a
                                title="重置等级"
                                href="javascript:"
