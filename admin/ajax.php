...
switch ($_QET['act']) {
    ...
    case 'app_view': //打开视图文件！
        ...
        AppList::view($_QET['id'], $_QET['path'], 1);
        break;
    case 'app_state_set':
    ...
    case 'app_prolong': //延长到期时间
        ...
        AppList::prolong($_QET['id'], (empty($_QET['value']) ? 1 : $_QET['value']));
        break;
    ...
    case 'app_pay': //购买
        if (empty($_QET['id'])) dies(-1, '请将参数填写完整');
        AppList::pay($_QET['id'], (empty($_QET['value']) ? 1 : $_QET['value']));
        break;
    ...
    case 'order_logistics': //填写订单物流单号
        ...
        if ($re) {
            Hook::execute('LogisticsDelivery', [
                'logistics' => $_QET['txt'], //物流规则
                'id' => $_QET['id'] //订单ID
            ]);
            CookieCache::del('OrderListCountAll');
            dies(1, '修改成功！');
    ...
    case 'BatchFreightTemEdit': //批量设置商品的运费模板
        ...
        if ($Res) {
            Hook::execute('GoodsBatchFreightTemEdit', [
                'arr' => $_QET['gid'],
                'freight' => $_QET['id'],
            ]);
            dies(1, '批量设置成功');
    ...
    case 'ChangeOrders': //修改订单状态
        ...
        $DB = SQL::DB();
        $re = $DB->get('order', ['uid', 'payment', 'order', 'state', 'price'], [
            'id' => (int)$_QET['did'],
        ]);
        if ($re['state'] == $_QET['id']) {
            dies(-1, '订单状态未改变，调整失败！');
        ...
        if ($_QET['id'] == 8) { //删
            ...
            if ($_QET['id'] == 5) { //退
                ...
                $Pre = 100;
                if (isset($_QET['type']) && $_QET['type'] == 2) {
                    //按照百分比扣除收益！
                    $Pre = $price / $re['price'] * 100;
                    if ($Pre >= 100) {
                        $Pre = 100;
                    }
                    if ($Pre <= 0) {
                ...
                if ($re['uid'] >= 1) {
                    if ($re['payment'] == '积分兑换') {
                        UserConf::DeductCommission($re['order'], '货币提成', $Pre);
                        ...
                        UserConf::DeductCommission($re['order'], '余额提成', $Pre);
                    ...
                    Hook::execute('OrderRefund', [
                        'order' => $re['order'], //订单编号
                        'type' => ($re['payment'] == '积分兑换' ? 1 : 2), //1=积分，2=其他
                        'price' => $price, //退款金额
                        'uid' => $re['uid'],//用户id
                        'date' => date('Y-m-d H:i:s') //退款时间
                    ]);
                } else {
                    if ($re['payment'] == '积分兑换') {
                    ...
                    $msg = '管理员已经将此订单设置为退款状态，退款订单号为：' . $re['order'] . '！';
                    $DB->update('order', [
                        'remark' => $msg,
                        'return' => '此订单已退单,退单数值为：' . $price,
                    ], [
                        'id' => $_QET['did']
                    ]);
                    Hook::execute('OrderRefund', [
    ...
    case 'set_order_all'://批量修改订单状态
        ...
        if ($_QET['type'] === 5) {
            ...
            try {
                $Count = UserConf::OrderBulkRefund($Res, $_QET['per'], $_QET['trye']);
            } catch (Exception $e) {
    ...
    case 'add_category': //添加/修改分类
        ...
        if (!empty($_QET['cid']) && $_QET['cid'] >= 1) {
            ...
            if ($_QET['superior'] == $_QET['cid']) {
                $_QET['superior'] = -1;
            }

            $Class = $DB->get('class', ['state', 'superior', 'cid'], ['cid' => $_QET['cid']]);
            if (!$Class) dies(-1, '分类不存在！');

            if ($Class['superior'] != $_QET['superior'] && $Class['superior'] == -1 && $_QET['superior'] >= 1) {
                //由一级分类变为其他分类的下级,所有下级分类跟随调整上级
                $DB->update('class', [
                    'superior' => $_QET['superior']
                ], [
                    'superior' => $Class['cid']
                ]);
            }
    ...
    case 'config_set': //保存网站信息
        $str = 'notice_top,notice_check,notice_bottom,notice_user,statistics,PopupNotice,ServiceTips,HostAnnounced,TradeMarkPublication,LoginAnnouncement';
        $Res = admin::config($_QET, $str, $_POST);
        if ($Res) {
            (new config())->unset_cache();
            dies(1, '保存成功,本次成功保存' . count($Res) . '个数据！');
        } else  dies(-1, '保存失败！');
    case 'login_scan': //获取登录二维码
        admin::login_scan();
        break;
    case 'login_app': //登录后台APP
        admin::login_fa(2);
        break;
    case 'login': //登录后台
        admin::login_fa();
        break;
    case 'login_account': //账号密码登录
        if (!VerificationCode::CaptchaValidation('Login_uvc', $_QET['vercode'])) {
            dies(-2, '验证码有误！');
        }
    ...
    case 'UpdateInspection': //站长数据+版本检测
        admin::index_update(($_QET['type'] == 2 ? 2 : 1));
        break;
    ...
    case 'image_up': //上传图片
        $images = PictureUpload::uploadPictureUser();
        dier([
            'code' => 0,
    ...
    case 'note_set': //商品备注信息
        ...
        if ($Res) {
            Hook::execute('GoodsModifyRemark', [
                'gid' => $_QET['gid'],
                'note' => $_QET['note'],
            ]);
            dies(1, '备注设置成功！');
    ...
    case 'config_set_sms': //保存短信配置
        if (empty($_QET['sms_switch_order']) || empty($_QET['Mobile'])) {
    ...
    case 'VerificationCode': //创建登录二维码图片
        if (empty($_QET['n'])) dies(-1, '请提交完整参数！');
        VerificationCode::RandomVerificationCode($_QET['n']);
        break;
    case 'Send_verification_code_login': //短信登录
        if (isset($_SESSION['ADMIN_PRODUCT_MS'])) {
            if ((time() - $_SESSION['ADMIN_PRODUCT_MS']) < 60) {
                dies(-1, '请勿频繁操作！');
            }
        }
        if (!VerificationCode::CaptchaValidation('AdminLogin_sms_vis', $_QET['code'])) {
            dies(-2, '验证码有误！');
        }
        if (empty($_QET['mobile'])) {
            dies(-1, '请将手机号填写完整！');
        }
    ...
    case 'OrderDel':
        if (empty($_QET['id'])) dies(-1, '请提交完整参数！');
        $id = (int)$_QET['id'];
        $DB = SQL::DB();
        $Res = $DB->delete('queue', [
            'id' => $id,
    ...
    case 'Tickets': //工单管理
        ...
        switch ($_QET['type']) {
            case 'list':
                $Limit = (int)$_QET['limit'] ?? 12;
                $Page = (($_QET['page'] ?? 1) - 1) * $Limit;
                $SQL = [];
                if (!empty($_QET['uid'])) {
                    $SQL['uid'] = $_QET['uid'];
                }
                ...
                $Count = $DB->count('tickets', $SQL);
                $SQL['ORDER'] = ['id' => 'DESC'];
                $SQL['LIMIT'] = [$Page, $Limit];
                $List = $DB->select('tickets', ['id', 'uid', 'name', 'class', 'type', 'state', 'addtime', 'grade'], $SQL);
                dier([
                    'code' => 0,
                    'msg' => '工单列表获取成功',
                    'data' => $List,
    ...
    case 'ToHot': //热卖商品统计
        ...
        switch ($_QET['type']) {
            case 'All':
                $Res = $DB->query("SELECT a.gid,a.name,(select count(id) from `sky_order` as b where b.gid = a.gid ) as count ,(select sum(money) from `sky_order` as b where b.gid = a.gid ) as cost,(select sum(price) from `sky_order` as b where b.gid = a.gid and b.payment != '积分兑换' ) as money FROM  `sky_goods` as a  ORDER BY `count` DESC LIMIT $page,$limit ")->fetchAll();
                $Count = $DB->count('goods');
                break;
            case 'Day':
                $Res = $DB->query("SELECT a.gid,a.name,(select count(id) from `sky_order` as b where b.gid = a.gid and  b.addtitm > '$DateDay' ) as count ,(select sum(money) from `sky_order` as b where b.gid = a.gid and  b.addtitm > '$DateDay' ) as cost,(select sum(price) from `sky_order` as b where b.gid = a.gid and b.payment != '积分兑换' and  b.addtitm > '$DateDay' ) as money FROM  `sky_goods` as a ORDER BY `count` DESC LIMIT $page,$limit ")->fetchAll();
                $Count = $DB->count('goods');
                break;
            case 'Yesterday':
