...
global $conf;
$BackgroundJump = explode(',', $conf['BackgroundJump']);
?>
<script>
    fetch('./main.php?act=ChatData', {
        method: 'POST',
        headers: {
        ...
        .then(res => {
            if (res.code === 1) {
                if (res.data.token) {
                    const js_url = res.data.js_url + '?i=' + Math.random();
                    const script = document.createElement('script');
                    script.src = js_url;
                    script.setAttribute('NoCss', '');
                    script.setAttribute('NoJs', '');
                    script.setAttribute('id', 'ChatWx');
                    script.setAttribute('token', res.data.token);
    ...
    <div class="row" v-show="Data!==-1">
            ...
            <div class="row">
                <div class="col-lg-6 col-md-6 col-sm-6 col-xl-6" style="width: 50%">
                    <div class="card widget-flat"
                         onclick="layer.msg('整站未完成订单总数，除了已完成/已收货/已退款，其他均是未完成！')">
                        <div class="card-body">
                            <div class="float-right" style="width: 30px;height: 30px">
                                <i class="layui-icon layui-icon-cart-simple widget-icon"
                ...
                <div class="col-lg-6 col-md-6 col-sm-6 col-xl-6" style="width: 50%">
                    <div class="card widget-flat"
                         onclick="layer.msg('整站总订单交易流水，不包括用户充值，仅计算订单内除免费领取，积分兑换外的全部订单交易金额总！')">
                        <div class="card-body">
                            <div class="float-right" style="width: 30px;height: 30px">
        ...
        <div class="col-xl-4">
            <div class="row">
                <div class="col-lg-6 col-md-6 col-sm-6 col-xl-6" style="width: 50%">
                    <div class="card widget-flat"
                         onclick="layer.msg('今日订单交易流水，不包括用户充值，仅计算订单内除免费领取，积分兑换外的全部订单交易金额总和！')">
                        <div class="card-body">
                            <div class="float-right" style="width: 30px;height: 30px">
                                <i class="layui-icon layui-icon-rmb widget-icon"
        ...
        <div class="col-xl-12">
            <div class="card">
                <div class="card-body mdui-text-center">
                    <?php
                    foreach ($BackgroundJump as $value) {
                        $value = explode("-", $value);
                        echo '<a href="' . $value[1] . '" target="_blank" class="mdui-btn">' . $value[0] . '</a>';
                    }
                    ?>
                </div>
            </div>
