...
`;
        ...
        }, RuleDataGet(type = 1) { //获取加价规则
            if (type === 1 && this.ListRule) {
                return;
            }
            $.ajax({
                type: "POST", url: './main.php?act=ListRule', dataType: "json", success: function (res) {
                    if (res.code == 1) {
                        let typees = false;
                        for (const key in res.data) {
                            if (res.data[key].id == App.Form.profit_rule) {
                                typees = true;
                            }
                        }
                        if (!typees) {
                            App.Form.profit_rule = -1;
                        }
                        App.ListRule = res.data;

                        if (type === 2) {
                            layer.msg('规则列表更新成功，共' + (res.data.length) + '条规则数据!', {icon: 1});
                        }
                    } else {
                        App.ListRule = [];
                    }
            ...
            //获取加价规则数据
            let Rules = false;
            if (this.Form.profit_rule >= 1) {
                try {
                    for (const listRuleKey in App.ListRule) {
                        if (App.ListRule[listRuleKey].id == App.Form.profit_rule) {
                            Rules = App.ListRule[listRuleKey];
                        }
                    }
                } catch (e) {
                    Rules = false;
            ...
            if (type === 1) {
                if (App.SellingPriceDataType[index] === 1 && App.SellingPriceData[index]['a'] != '') {
                    ...
                    //余额加价规则
                    if (Rules && Rules.json[index]) {
                        if (Rules.type == 1) {
                            result = (money + (Rules.json[index].price - 0)).toFixed(App.Form.accuracy);
                            App.profitArr[index] = (result - money).toFixed(App.Form.accuracy);
                        } else {
                            result = (money + (money * (Rules.json[index].price / 100))).toFixed(App.Form.accuracy);
                            App.profitArr[index] = (result - money).toFixed(App.Form.accuracy);
                        }
                        return '卖' + result + '元';
                    }
                ...
                if (App.SellingPriceDataType[index] === 1 && App.SellingPriceData[index]['b'] != '') {
                    ...
                    if (Rules && Rules.json[index]) {
                        if (Rules.type == 1) {
                            result = (money + (Rules.json[index].profits - 0)).toFixed(0);
                        } else {
                            result = (money + (money * (Rules.json[index].profits / 100))).toFixed(0);
                        }
                        return result + '积分';
                    }
                    result = money * profit;
  ...
  <table class="mdui-table">
    ...
    <tr>
        ...
        <td>` + this.InputRule[contentKey].placeholder + `</td>
            ...
            if (this.Form.extend.parameters === '') {
                mdui.dialog({
                    title: '温馨提示', content: `当前无可预览参数,请先点击旁边的<button onclick="App.ParametersAdd()" class="mdui-btn mdui-btn-icon mdui-text-color-orange" mdui-tooltip="{content: '添加一个新的商品自定义参数'}">
                        <i class="mdui-icon material-icons"></i>
                    </button>添加几个自定义参数吧!`, modal: false, history: false, buttons: [{
                        text: '关闭',
                    }]
                });
                return;
            ...
            let content = ``;
            let Array = this.Form.extend.parameters.split('|');
            for (const key in Array) {
                let arr = Array[key].split('_');
                let name = arr[0];
                let value = arr[1] || '无';
                content += `<p>` + name + `：` + value + `</p>`;
            }
            mdui.dialog({
                title: '商品自定义参数预览', content: content, modal: false, history: false, buttons: [{
                    text: '关闭',
                }],
            });
        }, ParametersAdd() { //添加一个新的标签
            let _this = this;
            let Content = `<div class="mdui-p-b-5"><div class="mdui-textfield">
      <label class="mdui-textfield-label">参数名称</label>
      <input class="mdui-textfield-input" id="Input1" type="text"/>
      <div class="mdui-textfield-helper">参数名称建议1-5字以内</div>
    </div>
    <div class="mdui-textfield">
      <label class="mdui-textfield-label">参数的值</label>
      <textarea class="mdui-textfield-input" id="Input2"></textarea>
      <div class="mdui-textfield-helper">请将参数的详细值填写完整！</div>
    </div></div>`;
            mdui.dialog({
                title: '添加商品自定义参数', content: Content, modal: true, history: false, buttons: [{
                    text: '关闭',
                }, {
                    text: '添加参数', onClick: function () {
                        const input1 = $("#Input1").val();
                        const input2 = $("#Input2").val();
                        if (input1 === "") {
                            layer.msg('请将参数名称填写完整!');
                            return false;
                        }
                        if (input2 == '') {
                            layer.msg('请将参数的值提交完整!');
                            return false;
                        }

                        let input = input1 + '_' + input2;

                        if (!_this.Form.extend.parameters) {
                            _this.Form.extend.parameters = '';
                        }

                        _this.Form.extend.parameters += ((_this.Form.extend.parameters === '') ? '' : '|') + input;
                        $("#parameters textarea").val(_this.Form.extend.parameters);
                        mdui.updateTextFields('#parameters');
                    }
                }], onOpened: function () {
                    mdui.mutation();
                }
...
<div id="tab2" class="mdui-p-a-0">
    ...
    <div class="mdui-textfield">
      ...
      <textarea class="mdui-textfield-input" id="Input3" placeholder="参数名称:参数值"></textarea>
      <div class="mdui-textfield-helper">可以回车换行,一行一个可选项,如果配置了参数值,则对接只会提交值</div>
    </div>
</div>
                `;
            mdui.dialog({
