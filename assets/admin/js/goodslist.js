        ...
        即此商品可赚取利润的百分比比例,基于用户等级,如当一个商品成本为100元时,可以在某个等级用户身上赚取50元的利润,可能50元对于此商品来说太多,用户不会购买此商品,用户等级利润又是固定死的,这时就可以通过调整商品的利润比例来进行价格微调!,如设置为50%,则仅从此用户身上赚25元,如果填写200%,则从此用户身上赚100元,50利润的200%,以此类推,当利润比例设置为0%时,则按照成本价出售商品!
            ...
            $.ajax({
                type: "POST", url: './main.php?act=GoodsList', data: {
                    page: App.page, limit: App.limit, name: App.name, cid: App.cid, sqid: App.sqid, state: App.state,
                }, dataType: "json", success: function (res) {
                    layer.close(is);
                    if (res.code == 1) {
                        App.Data = res.data;
...
<div class="mdui-textfield">
  ...
  <input class="mdui-textfield-input" id="input1" value="1" type="number"/>
                    ...
                    break;
                case 12: //加价规则
                    let cont = '<div style="width: 100%;" class="layui-elip"><a href="javascript:goods.BatchRule(\'' + arr.join('|') + '\',-1)" style="color: tomato">点击设置</a>：不设置加价规则</div>';
                    $.each(App.ListRule, function (key, val) {
                        cont += '<div style="width: 100%;" class="layui-elip"><a href="javascript:goods.BatchRule(\'' + arr.join('|') + '\',' + val.id + ')" style="color: tomato">点击设置</a>：' + val.name + ' | ' + val.id + '</div>';
                    });
                    mdui.dialog({
                        title: '批量设置加价模板', content: cont, modal: true, history: false, buttons: [{
                            text: '关闭',
                        },],
                    });
                    break;
                case 13: //批量换图
                    ...
                    break;
                case 14: //图片本地化
                    App.BatchProductPictureTransfer(arr);
                    break;
                default:
                    layer.alert('操作错误！');
                    break;
