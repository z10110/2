const App = Vue.createApp({
    data() {
        ...
        Calibration(id, index) {
            ...
            layer.open({
                title: '温馨提示', content: '是否要登录此空间的管理后台？', icon: 3, btn: ['确定登录', '取消'], btn1: function () {
                    let is = layer.msg('登录数据写入中，请稍后...', {icon: 16, time: 9999999});
                    $.ajax({
                        type: "POST",
...
<div class="mdui-textfield">
  ...
  <input class="mdui-textfield-input" id="endtime" value="` + data.endtime + `" type="text"/>
            ...
            mdui.prompt('可输入：用户ID，主机ID，主机标识，服务器ID，登录账号', '搜索主机', function (str) {
