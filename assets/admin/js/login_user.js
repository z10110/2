...
var AdminLogin = {
    IsPC() {
        ...
        layer.open({
            title: '是否要使用QQ互联登录？',
            content: '如果您是第一次使用QQ互联登录会自动帮您生成新账号哦！',
            btn: ['确认登录', '取消'],
            icon: 3,
            anim: 6,
            btn1: function () {
                location.href = '../user/ajax.php?act=QQ_QuickLogin';
                layer.msg('正在跳转登录中...', {icon: 16, time: 8999999})
            },
        });
    }, do_sms_verification() {
        /**
...
layui.use('form', function () {
    ...
    form.on('submit(user_register)', function (data) {
        layer.open({
            title: '注册确认',
            content: '您的注册信息如下：<br>登录账号：' + data.field['username'] + '<br>' + '登录密码：' + data.field['password'] + '<br>' + '绑定QQ：' + data.field['qq'] + '<br>' + '请确认信息,若有勿,点取消,重新填写！',
            btn: ['注册', '取消'],
            icon: 3,
            btn1: function () {
