const App = Vue.createApp({
    data() {
        ...
        OrderQueue() {
                ...
                case 5: //批量退款
                    layer.prompt({
                        ...
                        layer.open({
                            ...
                            icon: 3,
                            btn: ['退款(扣除分站全部收益)', '退款(根据百分比扣除分站收益)', '取消'],
                            btn1: function () {
                                order.ajax_state_all(type, arr, value, 1);
                            }, btn2: function () {
                                order.ajax_state_all(type, arr, value, 2);
                            }
                        })
        ...
        }, OrderSet(id, data, type = 1) {
            let did = data.id
            let money = data.price;
            if (id == 5) {
                let ntitle = (data.payment === '积分兑换' ? '积分' : '金额')
                layer.open({
                    title: '请配置退款' + ntitle + (type === 2 ? '[当前对接货源站已退单]' : ''),
                    ...
                    btn: ['退单(扣除全部分站收益)', '退单(按照百分比扣除分站收益)', '取消'],
                    btn1: function () {
                        layer.closeAll();
                        if ($("#Refund").text() === "") {
                            alert("请将退款金额提交完整！")
                        ...
                        order.ajax_state(id, did, $("#Refund").text() - 0);
                    }, btn2: function () {
                        layer.closeAll();
                        if ($("#Refund").text() === "") {
                            alert("请将退款金额提交完整！")
                        ...
                        order.ajax_state(id, did, $("#Refund").text() - 0, 2);
                    },
                    success: function () {
            ...
            let htmlinput = ``;
            let count = 0;
            for (const areaKey in content) {
                ++count;
                htmlinput += `
    <div class="layui-form-item">
        <label class="layui-form-label" id="NameInpit_` + areaKey + `">` + data.InputName[areaKey] + `</label>
        <div class="layui-input-block">
            <input type="text" id="Value_` + areaKey + `" value="` + content[areaKey] + `" name="title" required  class="layui-input">
        </div>
    </div>
                `;
            }
    ...
    ` + htmlinput + `  
            ...
            layer.open({
                ...
                btn1: function () {
                    ...
                    $.ajax({
                        type: "post", url: "ajax.php?act=set_order", data: {
                            id: id, input: JSON.stringify(arr)
                        }, dataType: "json", success: function (data) {
                            if (data.code > 0) {
                                layer.alert(data.msg, {
                                    icon: 1, end: function () {
                ...
                btn2: function () {
                    ...
                    let arr = [];
                    for (let i = 0; i < count; i++) {
                        arr[data.InputName[i]] = $("#Value_" + i).val();
                    }
                    let contentarr = [];
                    let i = 0
                    for (const arrKey in arr) {
                        console.log(arrKey);
                        contentarr[i] = arrKey + '：' + arr[arrKey];
                        ++i;
                    }
                    App.copyToClip(contentarr.join("，"), '下单信息复制成功！');
                }
            })
                    ...
                    type: "POST", url: './main.php?act=OrderCount', data: {
                        ...
                        if (res.code == 1) {
                            ...
                            laypage.render({
                                ...
                                theme: '#641ec6',
                                limit: App.limit,
                                limits: [1, 10, 20, 30, 50, 100, 200, 500, 1000, 2000, 3000, 5000, 10000],
                                groups: 3,
                                first: '首页',
                                last: '尾页',
                                prev: '上一页',
...
const order = {
    select_id: function (id) {
        ...
        return amount;
    }, ajax_state: function (id, did, money = 0, type = 1) {
        var index = layer.msg('修改中', {
            icon: 16, time: 9999999
        });
        $.ajax({
            type: "post", url: "ajax.php?act=ChangeOrders", data: {
                id: id, did: did, money: money, type: type
            }, dataType: "json", success: function (data) {
                if (data.code > 0) {
                    layer.alert(data.msg, {
                        icon: 1, end: function () {
        ...
        $.ajax({
            type: "post", url: "ajax.php?act=order_details", data: {
                ...
                if (res.code == 1) {
                    ...
                    if (Object.keys(data.Expansion).length > 0) {
                        $.each(data.Expansion, function (key, val) {
    ...
    }, ajax_state_all: function (type, arr, per = 0, trye = 1) {
        var index = layer.msg('批量操作中...', {
            icon: 16, time: 9999999
        });
        $.ajax({
            type: "post", url: "ajax.php?act=set_order_all", data: {
                type: type, arr: arr, per: per, trye: trye
            }, dataType: "json", success: function (data) {
                if (data.code > 0) {
                    layer.alert(data.msg, {
                        icon: 1, end: function () {
