const vm = Vue.createApp({
    data() {
        ...
        UpdateCache(type = 1) {
            layer.open({
                title: '温馨提示',
                content: (type == 1 ? '是否要更新货源列表?,若安装过其他货源对接插件,可以点击此按钮更新对接列表' : '是否需要同步货源对接文件,同步成功后会自动更新当前程序的货源对接文件，如果您发现某个货源站的对接api已更新，可以联系官方客服更新对接文件哦，更新成功后可以点击这个按钮来进行同步！'),
                icon: 3,
                btn: ['确定', '取消'],
                btn1: function () {
                    ...
                    $.ajax({
                        type: "POST",
                        url: './main.php?act=' + (type == 1 ? 'SourceCache' : 'SynchroSourceDockFile'),
                        data: {},
                        dataType: "json",
                        success: function (res) {
                            layer.close(is);
