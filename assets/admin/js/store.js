const App = Vue.createApp({
    data() {
        ...
        MenuSet(identification, type, state) {
            $.ajax({
                type: "POST", url: './main.php?act=AppMenuSet', data: {
                    ...
                    layer.msg('调整失败，请重新尝试！');
                }
            });
        },
        UpdateSoftwareList(type = 1) {
            if (type == 1) {
                layer.msg('正在更新应用列表，请稍后...', {icon: 16, time: 9999999});
            }
            $.ajax({
...
<div class="mdui-table-fluid">
  <table class="mdui-table">
   ...
   <br>
        ...
        UserGrade(data) {
            layer.prompt({
                title: '请输入需要调整的用户可用等级',
                value: data.user_grade,
                formType: 0
            }, function (value, index) {
                layer.close(index);
                let is = layer.msg('正在调整中，请稍后...', {icon: 16, time: 9999999});
                $.ajax({
                    type: 'post', url: './main.php?act=GradeSet', data: {
