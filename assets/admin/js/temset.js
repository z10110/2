const App = Vue.createApp({
    data() {
        ...
        TemplateSelection(index, data, type, state = 1) {
            let ConfName = ""
            if (state === 1) {
                ConfName = (type === 1 ? 'template' : 'template_m')
            } else {
                ConfName = (type === 1 ? 'UserBackgroundTemplatePC' : 'UserBackgroundTemplateM')
            }
            if (data === false) {
                let msg = '';
                if (index == -1) {
                ...
                layer.open({
                    title: '温馨提示', content: msg, icon: 3, btn: ['确定', '取消'], btn1: function () {
                        App.TemAjax(index, ConfName);
                    }
                })
            } else {
                let image = ''
                if (state == 2) {
                    image = '<img onerror="this.src=\'../assets/img/tebj.jpg\'" style="height:12em;max-width: 100%" src="../user/template/' + index + '/index.png"/>';
                } else {
                    image = '<img onerror="this.src=\'../assets/img/tebj.jpg\'" style="height:12em;max-width: 100%" src="../template/' + index + '/index.png"/>';
                }
                let content = '<div style="text-align: center">' + image + '</div><br>' + data.content + '<br><br>';
                ...
                mdui.dialog({
                    ...
                    buttons: [{
                        ...
                        text: '保存数据,并设置为' + (type == 1 ? 'PC端模板' : '移动端模板'), onClick: function () {
                            ...
                            App.TemAjax(index, ConfName, JSON.stringify(Json));
                        }
                    }],
                    onOpen: function () {
                        mdui.mutation();
