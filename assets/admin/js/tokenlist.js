...
const kami = {
    query: function () {
        ...
        if (App.gid === '') {
            layer.alert('请在下方选择需要导入库存的卡密商品!', {icon: 3});
            return false;
        }
        //开始上传文件
        layer.open({
            title: '请选择导入模式',
            icon: 3,
            content: '请确保导入文件内的文本格式为：<hr>卡密1<br>卡密2<br>卡密3<br>...<hr>一行一卡，文件格式为：txt格式，其他无法识别！',
            btn: ['导入', '导入[去重]', '取消'],
            success: function () {
                layui.use(function () {
                    ...
                    upload.render({
                        ...
                        url: './main.php?act=KamiLead&gid=' + App.gid + '&type=1',
                        accept: 'file',
                        done: function (res) {
                            layer.closeAll();
                            if (res.code === 1) {
                    ...
                    upload.render({
                        elem: '.layui-layer-btn1',
                        url: './main.php?act=KamiLead&gid=' + App.gid + '&type=2',
                        accept: 'file',
                        done: function (res) {
                            if (res.code === 1) {
                                layer.alert(res.msg, {
        ...
        layer.prompt({
            ...
            layer.alert('是否添加这' + content.length + '个卡密？', {
                title: '添加确认', icon: 3, btn: ['确定添加', '确定添加[去重]', '取消添加！'], btn1: function () {
                    $.post('./ajax.php?act=kami_add', {
                        gid: App.gid, kam_arr: content, type: 1
                    }, function (res) {
                        layer.closeAll();
                        if (res.code == 1) {
                            layer.alert(res.msg, {
                    ...
                    $.post('./ajax.php?act=kami_add', {
                        gid: App.gid, kam_arr: content, type: 2
                    }, function (res) {
                        layer.closeAll();
                        if (res.code == 1) {
                            layer.alert(res.msg, {
