const App = Vue.createApp({
    data() {
        ...
        EndTimeSet(uid, emdtime) {
            layer.open({
                title: '权限到期时间配置 - ' + uid,
                content: '请点击下方按钮设置到期时间，如点击，永久，则此用户的权限永远不会到期，如果点击时间，则可以开始设置权限到期时间！',
                btn: ['设置为永久', '设置时间'],
                btn1: function () {
                    App.Ajax(uid, 'endtime', -1)
                },
                success: function () {
                    App.laydate.render({
