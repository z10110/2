const vm = Vue.createApp({
    data() {
        return {
            ...
            Multiple: 1, //倍数
            MultipleNum: 1, //备份基数
            MultipleIndex: -1, //倍数输入框键值
        };
    }, mounted() {
        this.AjaxData();
    }, methods: {
        AdjustmentMultiple() {
            let index = this.form[this.MultipleIndex];
            if (index <= 1) {
                this.form[this.MultipleIndex] = 1;
            }
            this.Multiple = this.form[this.MultipleIndex];
        },
        CouponAdd(token) {
            let _this = this;
            ...
            layer.open({
                title: '登录或注册', type: 2, shadeClose: true, maxmin: true, //开启最大化最小化按钮
                area: area, content: './?mod=login_min&gid=' + vm.gid, end: function () {
                ...
                .then((res) => {
                    ...
                    if (data.code >= 0) {
                        if (thie.CouponOn.length >= 1 || thie.CouponOff.length >= 1) {
                            ...
                            <button ` + (data.data[3].surplus == "未登录，点击登录" || data.data[3].type != 1 ? 'style="display: none;"' : "") + ` class="btn btn-default btn-block" onclick="vm.submitpay(4)" style="margin-top:10px;">
                                <img src="assets/img/moneypay.png" class="logo">余额付款</button>
                            <button ` + (data.data[4].surplus == "未登录，点击登录" || data.data[4].type != 1 ? 'style="display: none;"' : "") + ` class="btn btn-default btn-block" onclick="vm.submitpay(5)" style="margin-top:10px;">
                        ...
                        `;
                            ...
                            content = `
                                ...
                                <button ` + (data.data[4].surplus == "未登录，点击登录" || data.data[4].type != 1 ? 'style="display: none;"' : "") + ` class="btn btn-default btn-block" onclick="vm.submitpay(5)" style="margin-top:10px;">
                                    <img src="assets/img/total.png" class="logo">积分兑换</button>
                                <hr ` + (data.data[3].surplus == "未登录，点击登录" || data.data[3].type != 1 ? 'style="display: none;"' : "") + `>
                                <div ` + (data.data[3].surplus == "未登录，点击登录" || data.data[3].type != 1 ? 'style="display: none;"' : "") + `>
                                    ` + data.data[3].surplus + `
                                </div>
                                <hr ` + (data.data[3].surplus == "未登录，点击登录" || data.data[3].type != 1 ? 'style="display: none;"' : "") + `>
                                <div ` + (data.data[4].surplus == "未登录，点击登录" || data.data[4].type != 1 ? 'style="display: none;"' : "") + `>
                                    ` + data.data[4].surplus + `
                                </div>
                                <hr>
                                <button class="btn btn-block" style="background: linear-gradient(to right, rgb(113, 215, 162), rgb(34, 215, 185)); font-weight: bold; color: white;" onclick="layer.closeAll();">取消付款</button>
                ...
                if (data.code >= 0) {
                    ...
                    if (Object.values(data.data.input).length >= 1) {
                        for (const key in data.data.input) {
                            ...
                            //绑定倍数输入框
                            let da = data.data.input[key];
                            if (da.state === 7) {
                                _this.MultipleNum = da.Data[1];
                                _this.MultipleIndex = key;
                                _this.form[key] = 1;
                            } else if (da.state === 4) {
                                try {
                                    _this.form[key] = da.Data[1][0];
                                } catch (e) {
                                    console.log('载入失败！')
                                }
                            }
                    ...
                    if (Object.values(data.data.input).length >= 1 && data.data.input[0].state == 5) {
                    ...
                    if (data.data.level_arr.length == 1) {
                        ...
                        $.each(data.data.level_arr, function (key, val) {
                            tips = '<span style="float:right;margin-right: 1em;color:seagreen;" >' + (val.price * thie.MultipleNum * thie.Multiple).toFixed(2) + "元</span>";
                        ...
                        $("#level").attr("onclick", "layer.alert('" + unescape(contents) + "',{title:'密价等级表',btn:['确定','升级密价'],shade: [0.8, '#000'],shadeClose:true,btn2:function(layero,index){vm.LoginIf();}})");
                    }
        ...
        NumRound(value, type = 1) {
            ...
            if (type === 1) {
                return num * (this.Multiple * this.MultipleNum);
            } else if (type === 3) {
                return value * (this.Multiple * this.MultipleNum);
            } else {
                if (num === 0) return 0;
                let str = num.toString();
                if (str.indexOf('.') !== -1) {
                    return str.split('.')[1].length * (this.Multiple * this.MultipleNum);
                }
                return 0;
            }
