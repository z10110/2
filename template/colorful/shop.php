    ...
    <div class="mdui-toolbar mdui-text-color-white"
        ...
        <?php } else { ?>
            <span style="margin-right:7px;width: 40px;min-width: 40px;height: 40px;"
                  ...
                  mdui-tooltip="{content: '<?= $conf['kfqq'] ?>',position: 'left'}">
                    <img id="headface" src="https://q4.qlogo.cn/headimg_dl?dst_uin=<?= $conf['kfqq'] ?>&spec=100"
                         width="100%">
                </span>
            <ul class="mdui-menu" id="user_menu">
                <li class="mdui-menu-item">
                    <a href="./user" target="_block" class="mdui-ripple">
                ...
                <div class="mdui-col-xs-12 mdui-col-sm-7 mdui-p-a-3">
                    ...
                    <h5 class="mdui-m-t-2" v-else>购买价格：
                        ...
                        <font v-else>
                            <font color="red"
                                  v-if="Goods.method == 1 || Goods.method == 2">
                                {{ ((NumRound(Goods.price) - NumRound(Goods.price * Goods.Seckill.depreciate) / 100) *
                                num).toFixed(Goods.accuracy) }}元
                            ...
                            <font v-else color="#ffaa00">
                                {{ NumRound((Goods.points - (Goods.points * Goods.Seckill.depreciate) / 100) *
                                num).toFixed(0) }}
                                {{ Goods.currency }}
                    ...
                    <div v-for="(item, index) of Goods.input" :key="index">
                        ...
                        <div class="mdui-textfield mdui-textfield-floating-label" v-if="item.state == 7">
                            <label class="mdui-textfield-label">{{item.Data[0]}}</label>
                            <input class="mdui-textfield-input" v-on:input="AdjustmentMultiple()"
                                   :placeholder="'请填写需要购买的'+item.Data[0]" type="number" v-model.number="form[index]"/>
                        </div>
