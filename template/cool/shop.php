...
<<!DOCTYPE html>
<html lang="zh"
      style="font-size: 20px;<?= !background::image() ? 'background:linear-gradient(to right, #bdc3c7, #2c3e50);' : background::image() ?>">
    ...
    <div class="fui-page  fui-page-current " style="overflow: inherit">
        <div id="container" class="fui-content " style="background-color: rgb(255, 255, 255); padding-bottom: 60px;">
                                            ...
                                            style="font-size: 1.2em">
                                                                                ...
                                                                                <font v-else>
                                                                                    <font color="red"
                                                                                        ...
                                                                                        {{ NumRound(Goods.price * num).toFixed(Goods.accuracy) }} 元
                                                                                    </font>

                                                                                    <font v-else color="#ffaa00">
                                                                                        {{ NumRound(Goods.points * num).toFixed(0) }}
                                    ...
                                    <span
                                            ...
                                            style="font-size: 1.2em">
                                                                                ...
                                                                                <font v-else>
                                                                                    <font color="red"
                                                                                          v-if="Goods.method == 1 || Goods.method == 2">
                                                                                       {{ ((NumRound(Goods.price) - NumRound(Goods.price * Goods.Seckill.depreciate) / 100) * num).toFixed(Goods.accuracy) }}元
                                                                                        <span style="color: #9e9e9e;text-decoration:line-through;font-size:12px;margin-left: 4px;">
                                                                                            {{ NumRound(Goods.price * num).toFixed(Goods.accuracy) }}
                                                                                        </span>
                                                                                    </font>

                                                                                    <font v-else color="#ffaa00">
                                                                                        {{ NumRound((Goods.points - (Goods.points * Goods.Seckill.depreciate) / 100) * num).toFixed(0) }}
                                                                                        {{ Goods.currency }}
                                                                                        <span style="color: #9e9e9e;text-decoration:line-through;font-size:12px;margin-left: 4px;">{{ NumRound(Goods.points * num).toFixed(Goods.accuracy) }}{{ Goods.currency }}</span>
                                                                                    </font>
                                                                                </font>
                                                                            </span>
            ...
            <marquee style="margin:1em;">
                <?php if (!$UserData) { ?>
                    <a href="./?mod=route&p=User" style="color: salmon">您当前未登录，点击此段文字进行登录后再下单，售后更加快捷方便哦~</a>
                <?php } ?>
            </marquee>
            ...
            <div style="height: 52vh;overflow:hidden;overflow-y: auto">
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-">
                        <input type="text" v-if="Goods.name != -1&&Goods.Seckill !== -1" disabled
                               style="color: red"
                               :value="((NumRound(Goods.price) - NumRound(Goods.price * Goods.Seckill.depreciate) / 100) * num).toFixed(Goods.accuracy)"
                               class="layui-input">
                        <input type="text" v-else disabled :value="NumRound(Goods.price * num).toFixed(Goods.accuracy) + '元'"
                               class="layui-input">
                    </div>
                </div>
                <div class="layui-form-item">
                ...
                <div v-for="(item, index) of Goods.input" :key="index">
                    ...
                    <div class="layui-form-item" v-if="item.state == 7">
                        <label class="layui-form-label"
                               style="width: 100%;text-align: left;padding:0">{{item.Data[0]}}</label>
                        <div class="input-group">
                                <span class="input-group-btn">
                                    <button @click="AdjustmentMultiple(1)" class="layui-btn layui-btn-sm"
                                            style="width: 2em;margin-top: -0.1em;border-radius: 0;">━</button>
                                </span>
                            <input v-on:input="AdjustmentMultiple(0)"
                                   class="layui-input"
                                   v-model.number="form[index]"
                                   :placeholder="'请填写需要购买的'+item.Data[0]"
                                   type="number" :min="1"/>
                            <span class="input-group-btn">
                                    <button @click="AdjustmentMultiple(2)" class="layui-btn layui-btn-sm"
                                            style="width: 2em;margin-top: -0.1em;border-radius: 0;">✚</button>
                                </span>
                        </div>
                    </div>
